...
- App.jsx — known (2026-07-31) — the root component, and the first file you wrote. Holds no data of its own since task 4.3 — it keeps `applications` in state, fetches them from the server once on load via useEffect, and renders them with `.map()`. The hardcoded array is gone; the server is now the single source. → [[react-component]] [[jsx]] [[list-rendering]] [[react-keys]] [[react-state]] [[useeffect]] [[fetch]] [[api]]
