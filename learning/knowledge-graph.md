...
## file-watching
- status: practicing
- depends-on: local-environment, nodejs
- introduced: 2026-08-05
- last-reviewed: 2026-08-05
- evidence: Task 7.2, after `node --watch` failed to restart on save four times (4.2, 6.3, 6.4, 7.1 — the last one making working validation look broken). Shown how watching works: the Linux kernel's **inotify** notifies a program when a registered file changes, and the two classic failure modes — the per-user watch limit (new watches fail *silently*), and editors that save by writing a temp file and **renaming** it over the original, leaving the watcher holding a reference to a replaced file. **Stated the differential diagnosis correctly and unprompted before running the test:** *"if save mechanism, then touch triggers restart; if watch limit then it doesn't"* — then ran `touch` and got a restart, confirming the editor was the cause. Fixed with `--watch-path=.`: a directory watch sees the rename-into-directory that a file watch misses. Also worked out why the client needed no change — Vite uses chokidar, which handles this, and its hot reload had never failed. Diagnostic habit named: when one tool misbehaves, check whether a similar tool on the same machine has the same problem.
...
- introduced: 2026-08-05
- last-reviewed: 2026-08-05
- evidence: Task 7.2: installed Prettier's VS Code extension, wrote `.prettierrc` (singleQuote, no semi — matching their existing style, on the grounds that consistency matters more than which convention) and `.vscode/settings.json` with formatOnSave, both committed so the rules travel with the repo. Predicted "the indentation" would change; shown the five other effects, including the one that matters for them — multi-prop JSX elements split one attribute per line, making "inside the tag or outside it" answerable at a glance. Also told to commit real work *before* the first format so the diff stays reviewable; did so, and used a `style(...)` commit type unprompted.
...
- introduced: 2026-07-31
- last-reviewed: 2026-07-31
- evidence: Scaffolded into the wrong folder (terminal was in learning/); ran `pwd` and correctly read off the cause — "client is now under job tracker, not learning" after fixing with `mv client ..`. Correctly predicted moving the folder would NOT break it, reasoning "the packages aren't directory specific". Task 1.4: ran `rm -r` on three paths, correctly predicting the running app would keep working "since theyre not used". Vocabulary met: pwd, cd, mv, rm -r, `..`, Ctrl+C. Told that rm has no OS-level undo and git is the only net. Task 2.1: ran `npm run dev` from the repo root and hit ENOENT on package.json — **second working-directory mistake of the project**. Diagnosed it themselves once pointed at the error's path. Reinforced: the terminal's location is invisible state; run `pwd` whenever a command misbehaves. **Task 4.2 — a genuinely useful debugging session:** the file on disk was correct but the live server wasn't sending the header. Diagnosed with `ps -eo pid,etime,cmd | grep node`, which showed `--watch` runs a *supervisor plus a child*, and the child was older than the edit. Fixed by a hard restart. Cause of the missed restart unknown — not chased. **Lesson named: a correct file and a correct running process are different things; verify at the source with curl, not through a browser.** **Task 7.5 — FOURTH working-directory incident, and the worst-behaved one.** Ran `npm install --save-dev supertest` from the repo root instead of server/. **npm doesn't error when there's no package.json — it creates one**, so a whole second project appeared at the root with 44 packages and no warning of any kind. Contrast with `npm run dev` in task 2.1, which *did* error because there was no script to look up. Cleaned up with absolute paths (`rm -rf` deserves them) and reinstalled after checking `pwd`. **Task 8.1 — a distinction that addresses the whole pattern.** Asked which directory to run `sudo dnf install` from. Answer: it doesn't matter, because **system commands act on the machine while project commands act on the directory you're standing in**. `npm install`, `npm run dev` and `node index.js` all operate on "here" — which is why the root install created a second project. `dnf` and `systemctl` operate on one machine-wide package database and service list. Rough tell: if it starts with `sudo`, location is almost certainly irrelevant. Also found a stray empty `job-tracker.db` at the root from a similar slip three days earlier — invisible because .gitignore matches the filename at any depth, which hides accidents as well as intended files.
...
- introduced: 2026-07-31
- last-reviewed: 2026-07-31
- evidence: Predicted correctly and with the right mechanism that removing `id="root"` would break the page — "wouldn't load since it wouldn't know where to find the html with id root". Made the edit, saw the blank page, opened DevTools console when prompted and reported the real error ("Target container is not a DOM element. at main.jsx:6:1"). Shown: getElementById returns null when it finds nothing, and the error's file/line is where code *crashed*, not where the bug *is*. Task 2.1: asked to predict a breakage and answered "i dont know" — honest, and the right trigger to go look rather than guess. Still needed a nudge to open the console (second time). Then read an ENOENT stack trace and diagnosed it unaided from the path ("needs to look in client folder"), and fixed "application is not defined" by spotting the singular/plural mismatch themselves. **Task 6.3 — clearest evidence yet of the loop working.** Given a `Cannot read properties of undefined (reading 'toUpperCase')` stack trace pointing at ApplicationCard.jsx:6, and after I'd checked the database and found the data clean, they said unprompted: *"i believe the error lies in line 51-60 in app.jsx"* — and were right. A stray `<ApplicationCard application={applications} />` was passing the whole array as a single application. Read the error, formed a hypothesis about a *different* file from the one named, checked it, found it. Also met: the console accumulates, so clear it before reproducing or you debug history. Earlier gap on reaching for the console unprompted looks closed. **Task 5.4: same gap in a new place** — reported `npm run dev` "works" when it was in fact crashing with their own thrown error; had been looking at the browser, not the server terminal. Named: when something doesn't work, the program's own output is the *first* place to look.
...
- introduced: 2026-07-31
- last-reviewed: 2026-07-31
- evidence: Task 2.4: opened the console when prompted and found the "each child should have a unique key prop" warning — note the page looked fine, so this only surfaced by looking. Proposed `company` as the key with sound reasoning ("the other fields could repeat"); shown the same-company-twice failure and that no descriptive field is a safe identifier. **First attempt wrote the key as an element — `<key = application.id>` — rather than a prop**; corrected by pointing at their own `application={...}` from task 2.2. Told that React consumes `key` so it never reaches props. Connected `id` to the database primary key coming in section 5. **Task 7.3: the nuance behind the "never use index" rule** — its reason is insertion, removal and reordering, so for a list rebuilt from scratch each render with no per-item state (the form's error messages) an index key would be harmless. General point made: rules encode a reason, and knowing the reason tells you when they apply. → [[tables-schema]]
...
- introduced: 2026-08-02
- last-reviewed: 2026-08-02
- evidence: Task 2.6: met the idea early via the status dropdown — shown the principle "make invalid states impossible to express" as better than validating after the fact. Also shown in 2.5 that `'offer'`/`'Offer'`/`'OFFER'` being three distinct values is the problem validation exists to remove. **Task 7.1: written.** Gave the first reason for validating unprompted — a usable message and the right status for the user. **Predicted correctly that `company: ""` would be accepted, with the right mechanism** ("it didn't give null it gave an empty string, but this still shouldn't be valid"), and later predicted 400-with-2-errors exactly. Wrote `validateApplication` returning a collected error list, and added the guard to PATCH. Key principles met: **a database enforces structure, not meaning** (status is just TEXT, so 'BANANA' is a fine string); **validate at the boundary**, with the schema as the last line of defence not the first; collect all errors rather than stopping at the first; and `return` after responding or the handler keeps running. **Misconception corrected: thought PATCH was "indirectly checked by post"** — routes don't chain, Express runs exactly one handler, so **every entry point needs its own guard**. Demonstrated live with curl.
...
- introduced: 2026-07-31
- last-reviewed: 2026-08-01
- evidence: Task 2.5: needed for the dynamic `className`. Struggled — first attempt had no backticks, wrong casing method, attribute in the wrong place, and dropped the visible content; the assembled line was ultimately handed over rather than derived. Did afterwards explain correctly why one copy of the status is lowercased and one isn't ("the one we're lowercasing may have capital letters"), sharpened to: one is read by CSS (case-sensitive), one by a person. **Task 4.4: failed a third time.** Wrote `"Server responded ${response.status}"` with double quotes, then `'...'` with single quotes, before getting backticks — needed to be told which physical key (left of `1`, above Tab). Separately wrote a backtick string inside JSX *children*, where it renders as literal characters and `{}` is the right tool. **The rule to over-learn: `${}` only means anything inside backticks; inside JSX text you use `{}` instead.** **Task 5.2 review after a two-day gap: PASSED (partial).** Asked cold when to reach for backticks, gave the interpolation answer correctly — "when you are writing a line that needs to execute javascript to obtain a value". Missed the second use, multi-line strings, which was sitting in their own db.js. Upgraded to practicing: first genuine retrieval after a real break. **Task 6.3: FOURTH failure, three attempts in one line** — wrote `%(id)`, then `?(id)`, then `$(id)` before landing on `${id}`. Named a likely cause: shell interference, since `$(command)` and `${VAR}` are both real bash syntax and they live in a terminal. **In a JS template literal it is always `${` … `}` — curly braces, never parentheses.** **Task 8.4: SIXTH failure** — both plain strings that needed *converting* to template literals kept their single quotes (`'${API_URL}/api/applications'`), while the two that were already template literals were fine. Pattern: recognising an existing template literal is solid; *converting* a plain string to one is not. **Repetition has stopped working — a structural fix was offered** (ESLint's `no-template-curly-in-string`, which flags exactly this) **and declined at the time; left parked in the plan.** **Task 8.3: FIFTH failure** — wrote `'An error occured, ${result.rowCount} rows affected'` in single quotes, and separately used single quotes for a two-line SQL string. Both rules still not automatic: `${}` needs backticks, and multi-line strings need backticks. **Task 6.4 review: PASSED** — asked cold to write the URL template literal from memory, produced `${id}` correctly (only slip: a missing closing backtick). Caveat: same-day recall after yesterday's correction, so the real test is the next session. Held at practicing.
...
- introduced: 2026-08-02
- last-reviewed: 2026-08-02
- evidence: Task 2.7: wired all five fields with `value` + `onChange` after one worked example. **Correctly predicted** that removing `onChange` while keeping `value` would freeze the box — "it will not update according to the user keystrokes, and will remain as its default value" — which is the round trip understood. Needed help with `<select>`: was thrown by it having children rather than being self-closing. **Task 6.4: a controlled input driven by props rather than local state** — the card's status dropdown takes its `value` from `props.application.status` and reports changes upward, so the server is the source of truth and the dropdown shows what's stored rather than what was typed. Shown: React as single source of truth, DOM merely displaying.
...
- introduced: 2026-08-02
- last-reviewed: 2026-08-02
- evidence: Task 2.7: shown that `{ company, role }` equals `{ company: company, role: role }`. Wrote the long form and left it that way after the shorthand was explained — shown, not adopted at the time. **Task 6.4: adopted unprompted** — wrote `JSON.stringify({ status })` without being reminded, three days after first being shown it. Upgraded to practicing.
...
- introduced: 2026-08-02
- last-reviewed: 2026-08-02
- evidence: Task 4.1: ran `fetch` from the browser console for the first time. Shown the two-await pattern and why it exists — the first await resolves when *headers* arrive, the second (`response.json()`) reads and parses the *body*, so `response` is the envelope and `.json()` opens it. Predicted the call would succeed; it was blocked by CORS instead (see [[cors]]). **Task 4.3: wrote fetch inside application code** — both awaits correct, result handed to `setApplications`, verified in the Network tab. Also spelled `reponse` consistently, which worked but was corrected on readability grounds. **Task 6.2: first POST from the browser** — `fetch(url, { method, headers, body })`, with `JSON.stringify` called by hand (the outbound half of the pair `res.json()` handles server-side). Needed the shape supplied rather than adapted from the section-4 GET. Also met the diagnostic table for "Failed to fetch": the *line above it* distinguishes a CORS block from `ERR_CONNECTION_REFUSED` from a bad hostname.
...
- evidence: Task 5.2: met INSERT, SELECT, WHERE and DELETE. **Wrote a WHERE query unaided** — `SELECT * FROM applications WHERE status = 'PENDING';` — correct syntax and correct row count (2). Hit `no such table: application` first and self-corrected: table names are exact. Shown two SQL-vs-JavaScript traps: `=` is comparison (there is no `==`), and double quotes mean *identifiers* not strings, so text needs single quotes. Told that `DELETE FROM x` with no WHERE removes every row. Task 8.2: met `RETURNING *` (Postgres returns the row it just wrote, id included, in one round trip — replacing the INSERT-then-SELECT pair in the POST route), column aliasing with `AS "dateApplied"`, and why an explicit column list beats `SELECT *`: `*` means "whatever columns happen to exist", so adding one silently changes the API response. Task 5.3: met the three better-sqlite3 methods — `.run()` (changes data), `.get()` (one row or undefined), `.all()` (array of rows) — and ran an UPDATE with a WHERE from the shell. Has not yet written ORDER BY or a JOIN.
...
- introduced: 2026-08-04
- last-reviewed: 2026-08-04
- evidence: Task 5.4: identified unprompted what the two hardcoded values had in common — "it assumes the url and database location/name" — leading to the rule that anything differing between environments is *configuration*, not logic, and must come from outside the program. **Chose fail-fast over a silent default when asked**, which is the right call and matters most for secrets (a fallback would let production come up healthy while writing to the wrong database). Created .env (gitignored) and .env.example (committed), and wired `--env-file` into the dev and seed scripts. Shown: `process.env` values are always strings; UPPER_SNAKE_CASE is the convention because hyphens break dot-access (they first wrote `DB-PATH`); `.env.example` exists so a fresh clone knows which variables are needed; and `start` deliberately omits `--env-file` because a production host injects variables directly. **Task 7.5: the payoff arrived early.** The DB_PATH switch built "for production" turned out to give a *third* environment — test — via `DB_PATH=test.db node --test`, with no change to db.js at all. Also used the inline `VAR=value command` form, the first of the three ways they'd been shown.
...
- introduced: 2026-08-05
- last-reviewed: 2026-08-05
- evidence: Task 6.1: wrote their first POST route. Shown that HTTP methods are *verbs declaring intent* — GET reads and is safe to repeat/cache/retry, POST creates and is not — and that nothing enforces this, but browsers, caches and back buttons all assume it. **Correctly predicted the 201 + created-row response** before running curl. Met `res.status(201).json(...)` chaining, and why the created row (with its generated id) must come back: the client can't edit or delete it otherwise. **Task 6.3: DELETE with a route parameter.** Shown the three places an id could go (query string, body, path) and why the path is conventional: **the URL is a noun, the method is a verb** — `/api/applications` is the collection, `/api/applications/5` is one application. That's REST. Met `:id` in the path and `req.params.id`, and that everything in a URL is text (SQLite converted `'19'` for them; Postgres won't). Used `info.changes` to distinguish "deleted" from "no such row", returning 204 (empty body, `.end()`) vs 404. **Key correction: `res.status(404)` on its own sends nothing** — the request hangs open, same failure mode as forgetting `next()` in middleware. **Task 6.4: PATCH.** Shown PUT (replace the whole resource — omitted fields are wiped) vs PATCH (change part), and why changing only status is a PATCH. Wrote the UPDATE with two positional `?` placeholders and was warned that order matters, unlike `@named`. Chose 200-with-the-row over 204 for the response, since unlike a delete there *is* something to describe. **Task 8.3: fourth time** — wrote `res.status(201)` alone in the DELETE handler (wrong code *and* nothing sent), then `res.status(204).json(row)` in the PATCH, which contradicts itself since 204 promises no body. **Recurring bug, third time: `res.status(204)` with nothing to send it** — the request hangs. Also passed `req.body` where `req.body.status` was needed (the whole object rather than the value inside it), twice in one lesson.
...
- introduced: 2026-08-05
- last-reviewed: 2026-08-05
- evidence: Task 6.1: the C of CRUD — create wired end to end from HTTP through a prepared INSERT to a row on disk, verified with curl and an id assigned by the database. R (read) was already done in section 5. **Task 6.3: D (delete) done** — server route plus a button on each card. **Task 6.4: U (update) done — CRUD complete.** All four operations working from the UI against a real database. The shape is now visible as a pattern: each operation is one HTTP method, one prepared statement, one handler that checks whether anything was affected, and (for the write operations) a callback from child to parent followed by a refetch. **Task 6.5 end-to-end trace check: partial.** Got the broad flow (client fetches a URL, server route runs, browser displays the result) but three mechanisms were wrong or missing: (1) described the URL as selecting the operation, where it's the **method** that does — same URL, different verbs; (2) **omitted the callback step entirely**, which is the only reason a card can trigger anything in App; (3) said the function "posts the updated data after the operation" and the browser displays it, but DELETE returns 204 with no body — the list refreshes because of a **separate GET**. Given the seven-step trace. **Re-check this in section 7.**
...
- introduced: 2026-08-05
- last-reviewed: 2026-08-05
- evidence: Task 6.2: **diagnosed the stale-list problem unaided** — "it only fetches once per refresh. we should make it fetch after a submission is made too" — correctly identifying the empty dependency array as the reason and proposing refetch-after-POST as the fix. Then wired it: `loadApplications` lifted out of the effect into the component body, passed down as `onApplicationAdded`, called by the form after a successful POST. **The idea: data flows down as props, events flow back up as callbacks** — a child can't reach into its parent, but the parent can hand it a function. Also shown the refetch-vs-append tradeoff and why refetch (one source of truth) beats append (one fewer request) until slowness is measured. Needed step-by-step help assembling all four pieces. **Task 6.4: third callback, two arguments** (`onStatusChange(id, newValue)`) — written correctly with much less help than 6.2 or 6.3. **Task 6.3: second time, with an argument** — `onDelete={handleDelete}` down, `onClick={() => props.onDelete(props.application.id)}` back up. Shown why the arrow wrapper is needed here but wasn't for `onSubmit`/`onApplicationAdded`: you can't pass an argument without calling, so you wrap the call in a function that runs later.
...
- evidence: Task 7.4: **named the strongest reason unprompted** when asked what breaks about manual checking — "need to check if adding new feature breaks another feature that was added in the past", i.e. regressions. Wrote the second test unaided (arrange → act → assert, behaviour-named). **Correctly predicted which test would fail** when the company check was commented out, then read the failure diff: `+ actual` / `- expected`, same convention as `git diff`. Principle demonstrated on their own code — they named regressions as the motivation, introduced one, and watched the test catch it in two seconds and point at the line. Also met: **a test you've never seen fail is not evidence**, since it may be asserting nothing. **Task 7.5: integration tests over HTTP.** Identified both obstacles unprompted — that importing index.js "tries to run a server", and that route tests would write to "our personal database, though we don't want them to". Met **test isolation** via `beforeEach` clearing the table, and the deeper reason for it: a test that depends on state it doesn't control (e.g. "GET returns 4 applications") fails for reasons unrelated to the code, so its failures stop meaning anything. Wrote two of the three route tests; the third had `info.length` where `info.body.length` belonged. **Reported "4 tests run, 4 should pass" when 5 ran and 1 failed** — the failure was mid-output; pointed at the `# pass` / `# fail` summary block as the thing to read. **Task 7.6: asked to name what five passing tests would NOT catch, and asked to be told rather than attempting it.** Supplied: the entire client, two of four routes, CORS (supertest isn't a browser and has no same-origin policy), rendering, and schema drift in the real database. Principle stated: **a green suite means "the things I checked still work", not "the app works"** — coverage is exactly what you point it at. Worth re-checking, since over-trusting green is the failure mode this guards against.
...
- introduced: 2026-08-06
- last-reviewed: 2026-08-06
- evidence: Task 7.4: extracted VALID_STATUSES, isNonEmptyString and validateApplication out of index.js into validation.js, because importing index.js to test a function would have started a web server. Principle named: **code that's hard to test is usually doing too much** — index.js was mixing wiring, logic and startup. Framed as better structure that testing *revealed*, not a concession to testing. Did the move cleanly and left `isNonEmptyString` unexported, so nothing outside can come to depend on it. **Task 7.5: the same principle one level up, and they spotted the problem themselves** — asked what goes wrong when a test imports index.js, answered "it tries to run a server". Consequences drawn out: port 3000 taken (EADDRINUSE — the error predicted in 3.2 and never seen), the test process never exiting because a server never finishes, and a real network port being heavy machinery for the question. Split into `app.js` (build and configure, export) and `index.js` (import and listen, three lines). The entry point now does only the job that differs in production.

## copying-vs-reading-code
- status: introduced
- depends-on: none
- introduced: 2026-08-07
- last-reviewed: 2026-08-07
- evidence: Task 8.3, worth watching. Asked to write an INSERT inside a seed **script**, pasted the POST **route handler** wholesale — including `req.body.company` (there is no request) and `res.status(201).json(...)` (there is nobody to reply to). The query itself was right; what came with it wasn't. Named: **the query is identical, only the source of the values changes** — `application.company` in a script, `req.body.company` in a route. The general habit: when copying code, ask which parts belong to the *context* it came from rather than the *job* being done. Also asked for the answer rather than attempting several fill-ins in this lesson (a long one); flagged not as a criticism but because attempting-and-being-wrong is where most of the evidence in this graph came from.
...
- introduced: 2026-08-07
- last-reviewed: 2026-08-07
- evidence: Task 7.5: used `{ ...validApplication, company: '' }` in a test — copy every property, then override one. Framed as "the same as that, but with this changed", which keeps a test saying exactly what it varies and keeps it correct if fields are added later. Used once, from a supplied example.
...
- introduced: 2026-08-07
- last-reviewed: 2026-08-07
- evidence: Task 8.1: agreed to change a locked decision after understanding the argument — SQLite locally + Postgres in production would mean the code meets the production database for the first time *on deploy day*, with dialect, strictness and driver differences all surfacing at once (including a live bug in the delete route, where SQLite silently converts `'19'` to `19` and Postgres won't). Principle named: **dev/prod parity**. Asked what changes when the database is a *server* rather than a file, answered "need another port, change path on all requests" — correct as far as it went; extended with: it must be running, it needs credentials (hence a password in DATABASE_URL), and the app no longer owns the data but asks a program that arbitrates between clients — which is *why* Postgres exists where SQLite struggles. Installed 18.3, initialised the data directory, enabled the service. **Hit `FATAL: role "samir" does not exist` and read it properly**: Postgres has its own user system separate from Linux's, local connections go over a Unix socket rather than the port, and peer authentication trusts the OS's word about identity provided a matching role exists. Created a role via `sudo -u postgres`, made the database, connected. Told that `--superuser` is fine locally but production wants a least-privilege role. **Task 8.2 — three dialect differences, two found by wrong predictions.** (1) Predicted the SQLite `CREATE TABLE` would *fail* in Postgres; it **succeeded** — valid syntax, different meaning, which is the dangerous kind of incompatibility because nothing tells you. (2) **Postgres folds unquoted identifiers to lowercase**, so `dateApplied` became `dateapplied`; asked what a user would then see, predicted "same thing" — traced instead to `props.application.dateApplied` being `undefined`, rendering as blank, no error: a silent failure of exactly the kind named in 7.3, and one that under the old plan would have appeared first *in production*. (3) **Correctly predicted `INTEGER PRIMARY KEY` would not generate ids** and diagnosed the error unaided — "postgres isn't generating id"; SQLite's auto-assignment is the special case, Postgres needs `GENERATED ALWAYS AS IDENTITY` (or the older `SERIAL`). Chose snake_case columns plus `AS "dateApplied"` aliasing over quoting identifiers everywhere, on the reasoning that it's conventional, has no remember-to-quote trap, and makes the database-naming/API-naming boundary explicit. Also met `RETURNING *`. One slip: omitted the comma after the id column definition. **Task 8.3 — the driver port.** Met the `pg` API against better-sqlite3's: one `pool.query(text, values)` instead of prepare/get/all/run; `result.rows` always an array; `result.rowCount` replacing `changes`; `$1, $2` positional placeholders (a third style after `@name` and `?`); `RETURNING` collapsing insert-then-select into one query; and every handler becoming `async` because the driver is. Also met **top-level await** in ESM (the CREATE TABLE finishes before anything imports db.js) and that **a pool holds connections open**, so a script needs `pool.end()` or it never exits — the inverse of the task-3.2 lesson that a server is supposed to never finish. Wrote the Postgres schema into db.js **from memory**, correctly. Local auth moved from peer-over-socket to password-over-TCP by editing `pg_hba.conf` (backed up first, two lines changed, `reload` not `restart`) so the local connection has the same *shape* as production's.
...
- introduced: 2026-08-07
- last-reviewed: 2026-08-07
- evidence: Task 8.3: asked what's wrong with opening a connection per request, answered "not efficient" — right, but the specifics were supplied: you pay a TCP handshake and authentication on every request (often longer than the query), and Postgres has a hard cap on simultaneous connections (~100 by default, each costing memory), so under load you get *failures*, not just slowness. A pool opens a few connections once and lends them out. Told, not yet retrieved.
...
- introduced: 2026-08-07
- last-reviewed: 2026-08-07
- evidence: Task 8.3: DATABASE_URL replaced DB_PATH, and for the first time an environment variable holds **a real secret** — the reason `.env` was gitignored back in 5.4, now concrete rather than hypothetical. Met the connection-string format (`postgresql://user:password@host:port/database`) as five pieces of configuration in one string, identical in shape to what Render will provide. `.env.example` carries the key with an empty value, exactly as described in 5.4. Also noticed why the 7.5 approach no longer worked: `DB_PATH=test.db` could sit in package.json because a filename isn't a secret; a connection string can't, so tests moved to a gitignored `.env.test`. **Task 8.4: the client/server split, and it's a security distinction.** Asked whether the client's `.env` is the same kind of secret as the server's, answered "client env will hold passwords to frontend hosting" — wrong on both counts. Corrected: deployment credentials live in the hosting account, never in app code; and **a client environment variable is not secret at all**, because Vite bakes the value into the bundle at build time and every visitor downloads it. That's why Vite ignores anything without a `VITE_` prefix — an explicit opt-in so server secrets can't leak into a browser bundle by accident. Rule stated: **if a value must stay secret it can never live in client code**; anything the browser needs that involves a secret must go through the server. Consequence they applied: `client/.env.example` carries the actual value (nothing to withhold) while `server/.env.example` leaves DATABASE_URL blank.
...
- introduced: 2026-08-07
- last-reviewed: 2026-08-07
- evidence: Task 8.3: the five tests written in section 7 were the instrument that verified the largest change in the project — a whole database engine and driver swapped underneath four routes. They failed loudly on the un-ported `beforeEach` (`db.exec is not a function`, three times), then went green once the port was complete. The argument made abstractly in 7.4 ("regressions") was demonstrated at full scale two days later. **Task 8.4 showed the other face within the hour:** writing `cors({...})` without wrapping it in `app.use(...)` switched CORS off entirely — every browser request would fail — and **all five tests stayed green**, because supertest isn't a browser. That's the exact coverage gap listed in 7.6, producing a real bug the same week it was written down.
...
- introduced: —
- last-reviewed: —
- evidence: Flagged in task 5.5 as the reason `CREATE TABLE IF NOT EXISTS` isn't enough long-term: it silently does nothing when the table exists, so adding a column leaves db.js and the on-disk schema disagreeing. Crude workaround for now (delete the .db and re-seed) is fine with fake data only. **Task 8.2: met it directly** — `DROP TABLE` was the right move because the data was disposable, and named as the thing that stops being acceptable the moment the rows matter. Still not solved: no migration tooling, and `CREATE TABLE IF NOT EXISTS` remains silent when a table exists but has the wrong shape.
